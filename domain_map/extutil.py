...
def creturn(status_code, progress, success=None, error=None, logs=None, pass_back_data=None, state=None, props=None, links=None, callback_sec=2, error_details={}, artifacts=None):
...
class ExtensionHandler:
    ...
    def declare_pass_back_data(self, pass_back_data):
        ...
        self.artifacts = pbd.pop("artifacts", {}) or {}
        print(f"Ops = {self.ops}, Retries = {self.retries}, Links = {self.links}, Props = {self.props}, Artifacts = {self.artifacts}, State = {self.state}")
        self.children = pbd
        if pbd:
                ...
                if error:
                    ...
                    if op == "upsert":
                        if not ignore_props_links:
                            ...
                            # Making it so that props and artifacts behave the same way.
                            if artifacts:
                                if isinstance(self.artifacts.get(child_key), dict):
                                    self.artifacts[child_key].update(artifacts)
                                else:
                                    self.artifacts[child_key] = artifacts
                                if merge_props:
